            ...
            {user?.length > 0 ? (
                <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                    {user?.map((item) => (
                        <div
                            key={item.id}
                            className=" shadow p-5 text-center rounded">
                            {editId === item.id ? (
                                <input
                                    value={editName}
                                    onChange={(e) =>
                                    ...
                                    className="border p-1 rounded w-full text-2xl font-bold min-h-18"
                                />
                            ) : (
                                <h4 className="font-bold text-2xl min-h-18">
                                    {item?.name}
                                </h4>
                            )}
                            <h5 className="font-semibold text-gray-700 min-h-16">
                                {item?.email}
                            </h5>
                            <h5 className=" font-semibold min-h-14">
                            ...
                            <div className="flex gap-5 justify-center mt-4">
                                ...
                                {editId === item.id ? (
                                    <button
                                        className="border bg-green-700 text-white px-6 rounded cursor-pointer"
                                        onClick={() => handleSave(item.id)}>
                                        Save
                                    </button>
                                ) : (
                                    <button
                                        className="border bg-blue-500 text-white px-7 py-1 rounded cursor-pointer"
                                        onClick={() =>
